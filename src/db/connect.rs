...
pub async fn postgres_connect(config: &Config) -> Result<DatabaseConnection> {
    let connect_info = &config.postgres.connect_info;
    ...
    let runtime_options = &config.postgres.runtime_options;
